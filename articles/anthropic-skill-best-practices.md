...
- **两侧悬崖的窄桥**：只有一条安全的前进方式。提供明确的限制和精确的指令（低自由度）。示例：必须按确切顺序执行的数据库迁移。
- **开阔无危险的田野**：许多路径都能通向成功。给出大方向并信任 Claude 找到最佳路线（高自由度）。示例：依赖上下文判断最佳方法的代码审查。
...
对于每个主要论点，验证它在源材料中有据可查。记录每个观点对应的来源。
...
检查每个论点是否引用了正确的源文档。如果引文不完整，请返回步骤 3。
```
...
在编写广泛的文档之前创建评估。这确保你的 Skill 解决真实存在的问题，而不是文档化一些想象中的需求。
...
评估是衡量 Skill 有效性的可靠基准。

### 与 Claude 一起迭代开发 Skill

最有效的 Skill 开发过程涉及 Claude 本身。与一个 Claude 实例（"Claude A"）合作创建将由其他实例（"Claude B"）使用的 Skill。Claude A 帮助你设计和完善说明，而 Claude B 在真实任务中测试它们。这样做之所以有效，是因为 Claude 模型既能理解如何编写有效的 Agent 指令，也能理解 Agent 执行任务所需的信息。
...
**为什么这种方法有效**：Claude A 了解 Agent 的需求，你提供领域专业知识，Claude B 通过真实使用揭示不足，迭代优化基于实际观察的行为，而非主观假设。
...
配置参数也应该有合理的依据并加以注释说明，以避免出现"魔法常量"（Ousterhout 定律）。如果你自己都不知道正确的值，Claude 就更没法确定了。
...
- **文件名要有描述性**：使用能体现内容的名称：`form_validation_rules.md`，而不是 `doc2.md`
- **为发现而组织**：按域或功能组织目录
  - 好的：`reference/finance.md`、`reference/sales.md`
  - 坏的：`docs/file1.md`、`docs/file2.md`
- **打包完整资源**：包括完整的 API 文档、丰富的示例、大型数据集；未被读取前不占用上下文 token
